/*
 * libiio - AD7768-1 IIO streaming example
 *
...
#define CHANNEL_NUMBER 0  // AD7768-1 is a single channel variant of AD7768
...
/* returns AD7768-1 device */
static struct iio_device* get_ad7768_1(struct iio_context *ctx)
{
	struct iio_device *dev =  iio_context_find_device(ctx, "ad7768-1");
	IIO_ENSURE(dev && "No ad7768-1 found");
	return dev;
}

/* finds AD7768-1 streaming IIO devices */
static bool get_ad7768_1_stream_dev(struct iio_context *ctx, enum iodev d, struct iio_device **dev)
{
	switch (d) {
	case RX: *dev = iio_context_find_device(ctx, "ad7768-1");  return *dev != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* finds AD7768-1 streaming IIO channels */
static bool get_ad7768_1_stream_ch(__notused struct iio_context *ctx, enum iodev d, struct iio_device *dev, int chid, struct iio_channel **chn)
{
	*chn = iio_device_find_channel(dev, get_ch_name("voltage", chid), false);
	return *chn != NULL;
}

/* finds AD7768-1 IIO configuration channel with id chid */
static bool get_channel(struct iio_context *ctx, enum iodev d, int chid, struct iio_channel **chn)
{
	switch (d) {
	case RX: *chn = iio_device_find_channel(get_ad7768_1(ctx), get_ch_name("voltage", chid), false); return *chn != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* applies streaming configuration through IIO */
bool cfg_ad7768_1_streaming_ch(struct iio_context *ctx, struct stream_cfg *cfg, enum iodev type, int chid)
{
	struct iio_channel *chn = NULL;

	// Configure channels
	printf("* Acquiring AD7768-1 channel %d\n", chid);
...
int main (int argc, char **argv)
{
	...
	printf("* Acquiring AD7768-1 streaming devices\n");
	IIO_ENSURE(get_ad7768_1_stream_dev(ctx, RX, &rx) && "No rx dev found");

	printf("* Configuring AD7768-1 for streaming\n");
	IIO_ENSURE(cfg_ad7768_1_streaming_ch(ctx, &rxcfg, RX, CHANNEL_NUMBER) && "RX port not found");

	printf("* Initializing AD7768-1 IIO streaming channels\n");
	IIO_ENSURE(get_ad7768_1_stream_ch(ctx, RX, rx, CHANNEL_NUMBER, &rxchan) && "RX chan not found");
	...
	while (!stop)
	{
		...
		for (sample = 0; sample < bytes / sample_size; ++sample)
			printf("Buffer Sample: %ld\tCH%d Data: 0x%x\n",
					sample, CHANNEL_NUMBER, ((int32_t *)buf)[sample] >> 8);	// Shift by 8 bits to match the output format
