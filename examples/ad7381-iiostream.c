/*
 * libiio - AD7381 IIO streaming example
 *
 * Based on AD7380 example
 *
...
#define CHANNEL_NUMBER 0  // AD7381 is a 14-bit variant of AD7380
...
/* returns AD7381 device */
static struct iio_device* get_ad7381(struct iio_context *ctx)
{
	struct iio_device *dev =  iio_context_find_device(ctx, "ad7381");
	IIO_ENSURE(dev && "No ad7381 found");
	return dev;
}

/* finds AD7381 streaming IIO devices */
static bool get_ad7381_stream_dev(struct iio_context *ctx, enum iodev d, struct iio_device **dev)
{
	switch (d) {
	case RX: *dev = iio_context_find_device(ctx, "ad7381");  return *dev != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* finds AD7381 streaming IIO channels */
static bool get_ad7381_stream_ch(__notused struct iio_context *ctx, enum iodev d, struct iio_device *dev, int chid, struct iio_channel **chn)
{
	*chn = iio_device_find_channel(dev, get_ch_name("voltage", chid), false);
	return *chn != NULL;
}

/* finds AD7381 IIO configuration channel with id chid */
static bool get_channel(struct iio_context *ctx, enum iodev d, int chid, struct iio_channel **chn)
{
	switch (d) {
	case RX: *chn = iio_device_find_channel(get_ad7381(ctx), get_ch_name("voltage", chid), false); return *chn != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* applies streaming configuration through IIO */
bool cfg_ad7381_streaming_ch(struct iio_context *ctx, struct stream_cfg *cfg, enum iodev type, int chid)
{
	struct iio_channel *chn = NULL;

	// Configure channels
	printf("* Acquiring AD7381 channel %d\n", chid);
...
int main (int argc, char **argv)
{
	...
	printf("* Acquiring AD7381 streaming devices\n");
	IIO_ENSURE(get_ad7381_stream_dev(ctx, RX, &rx) && "No rx dev found");

	printf("* Configuring AD7381 for streaming\n");
	IIO_ENSURE(cfg_ad7381_streaming_ch(ctx, &rxcfg, RX, CHANNEL_NUMBER) && "RX port not found");

	printf("* Initializing AD7381 IIO streaming channels\n");
	IIO_ENSURE(get_ad7381_stream_ch(ctx, RX, rx, CHANNEL_NUMBER, &rxchan) && "RX chan not found");
