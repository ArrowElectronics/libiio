/*
 * libiio - AD7380 IIO streaming example
 *
...
#define CHANNEL_NUMBER 0  // AD7380 has 2 channels
...
/* returns AD7380 device */
static struct iio_device* get_ad7380(struct iio_context *ctx)
{
	struct iio_device *dev =  iio_context_find_device(ctx, "ad7380");
	IIO_ENSURE(dev && "No ad7380 found");
	return dev;
}

/* finds AD7380 streaming IIO devices */
static bool get_ad7380_stream_dev(struct iio_context *ctx, enum iodev d, struct iio_device **dev)
{
	switch (d) {
	case RX: *dev = iio_context_find_device(ctx, "ad7380");  return *dev != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* finds AD7380 streaming IIO channels */
static bool get_ad7380_stream_ch(__notused struct iio_context *ctx, enum iodev d, struct iio_device *dev, int chid, struct iio_channel **chn)
{
	*chn = iio_device_find_channel(dev, get_ch_name("voltage", chid), false);
	return *chn != NULL;
}

/* finds AD7380 IIO configuration channel with id chid */
static bool get_channel(struct iio_context *ctx, enum iodev d, int chid, struct iio_channel **chn)
{
	switch (d) {
	case RX: *chn = iio_device_find_channel(get_ad7380(ctx), get_ch_name("voltage", chid), false); return *chn != NULL;
	default: IIO_ENSURE(0); return false;
	}
}

/* applies streaming configuration through IIO */
bool cfg_ad7380_streaming_ch(struct iio_context *ctx, struct stream_cfg *cfg, enum iodev type, int chid)
{
	struct iio_channel *chn = NULL;

	// Configure channels
	printf("* Acquiring AD7380 channel %d\n", chid);
...
int main (int argc, char **argv)
{
	...
	printf("* Acquiring AD7380 streaming devices\n");
	IIO_ENSURE(get_ad7380_stream_dev(ctx, RX, &rx) && "No rx dev found");

	printf("* Configuring AD7380 for streaming\n");
	IIO_ENSURE(cfg_ad7380_streaming_ch(ctx, &rxcfg, RX, CHANNEL_NUMBER) && "RX port not found");

	printf("* Initializing AD7380 IIO streaming channels\n");
	IIO_ENSURE(get_ad7380_stream_ch(ctx, RX, rx, CHANNEL_NUMBER, &rxchan) && "RX chan not found");
	...
	while (!stop)
	{
		...
		if (!CHANNEL_NUMBER) {
			for (sample = 0; sample < bytes / sample_size; ++sample)
				printf("Buffer Sample: %ld\tADC0: 0x%x\n", sample, (int16_t)((int64_t *)buf)[sample]);
		}
		else {
			for (sample = 0; sample < bytes / sample_size; ++sample)
				printf("Buffer Sample: %ld\tADC1: 0x%x\n", sample, (int16_t)(((int64_t *)buf)[sample] >> 32));
		}
